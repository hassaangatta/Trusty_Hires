...
const ApplicantEditInfo = ({ preInfo,username,setEditInfo }) => {
    ...
    const [city, setcity] = useState(preInfo.city);
    const [state, setstate] = useState(preInfo.state);
    const [country, setcountry] = useState(preInfo.country);
    ...
    const validateForm = () => {
        const newErrors = {
          ...
          locations: (city.trim() === '' || state.trim() === '' || country.trim() === '') ? 'Location field is required ' : '',
        };
        setErrors(newErrors);
    ...
    const updateprofile = async (body) => {
        try {
            const response = await fetch("http://localhost:5000/updateapplicantprofileinfo", {
                method: "PUT",
    ...
    const getImage = async () => {
        ...
        // console.log(i);
        await setimg(i !== '' ? i:require(`../Images/temp_profile.png`))
        await setimgPresent(i !== '' ? true:false)
        } catch (err){  
            console.error(err.message);
        }
      }
    ...
    },[setEditInfo,updateprofile,username,validateForm,name,email,contact,linkedin,city,state,country]);
...
export default ApplicantEditInfo;
